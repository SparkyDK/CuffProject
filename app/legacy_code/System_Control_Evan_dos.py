...
with open(filename, 'r') as f:
    ...
    if (num_lines > Max_num_schedules):
        ...
        raise ValueError("Only a total of ", Max_num_schedules, " schedule statement lines are allowed in the file [",
                         num_lines, " lines were detected]")
    ...
    for i in range(0, Max_num_schedules):
        ...
        if (value >= 0 and value <= 999):
            ...
            raise ValueError("Interval values, measured in seconds, must be in range [0,999], but a value of [", value,
                             "] was detected")
...
with open(filename, 'r') as file:
    ...
    if (Patm < Plow and Plow < Pup and Pup < Pmax):
        ...
        print
        "Pmax=", Pmax, " Pup=", Pup, " Plow=", Plow, " and Patm=", Patm
    else:
        raise ValueError("Need Pmax > Pup > Plow > Patm but, instead, have Pmax=", Pmax, "Pup=", Pup, " Plow=", Plow,
                         " and Patm=", Patm)
...
def main_code(*args, **kwargs):
    ...
    if (DEBUG == True and True == False):
        ...
        if (schedule_index < Max_num_schedules):
            print
            localtime, " (elapsed=", elapsed_time, "): CNT=", Global_cnt, "State=", c.FSM.GetCurState(), "schedule_started=", schedule_started, "GO=", GO, " STOP=", STOP, "ABORT=", ABORT, "PAUSE=", PAUSE, "Pain=", Pain, "P=", Current_Pressure, "Pain_dex=", schedule_index, "Pain_cnt=", \
            Current_counter[schedule_index]
        else:
            print
            localtime, " (elapsed=", elapsed_time, "): CNT=", Global_cnt, "State=", c.FSM.GetCurState(), "schedule_started=", schedule_started, "GO=", GO, " STOP=", STOP, "ABORT=", ABORT, "PAUSE=", PAUSE, "Pain=", Pain, "P=", Current_Pressure, "Pain_dex=", schedule_index,
    ...
    if (second_tickover == True):
        ...
        if (P > Pmax):
            print
            "Pressure value (", P, ") exceeds max value (", Pmax
    ...
    if (DEBUG == True and (Global_cnt == 500 or Global_cnt == 4000 or Global_cnt == 5000)):
        ...
        GO = 1
        print
        "\tDebug: Go activated with Global count = ", Global_cnt

    if (DEBUG == True and (Global_cnt == 1000 or Global_cnt == 4500)):
        # debug statement only!
        GO = 0
        print
        "\tDebug: Go de-ctivated with Global count = ", Global_cnt
        ...
        if (schedule_index < Max_num_schedules):
            ...
            if (Current_counter[schedule_index] > 1):
                ...
                if (second_tickover == True):
                    ...
                    print
                    "Schedule Counter adjusted: Schedule:", schedule_index, " with counter value = ", Current_counter[
                        schedule_index]
            else:
    ...
    if (DEBUG == True and Global_cnt == 20):
        ...
        STOP = 1
        print
        "\tDebug: Stop activated with Global count = ", Global_cnt

    if (DEBUG == True and (Global_cnt == 30 or Global_cnt == 2)):
        # debug statement only!
        STOP = 0
        print
        "\tDebug: Stop de-activated with Global count = ", Global_cnt
    ...
    if (Pain == 'PAIN'):
        ...
        args[3][3] = False
        print
        "Error!: Pain has a value of ", Pain, " which is neither PAIN or NILL"

    if ((Pain_signal != old_pain) or (schedule_started == True and old_schedule_started == False)):
        timediff = time.time() - pain_time
        print
        "Pain signal changed to ", Pain_signal, "after ", round(timediff), "(", timediff, ") seconds"
        pain_time = time.time()
    ...
    if (DEBUG == True):
        ...
        if (past_states[4] == "CONNECT_CUFF" and past_states[3] == "ISOLATE_RESERVOIR" and past_states[
            2] == "LOAD_RESERVOIR"):
    ...
    if (PAUSE == True):
        ...
        pause_value = 0
    control_args = {'GO': GO, 'STOP': STOP, 'PAIN': Pain_signal, 'P': Current_Pressure, 'RUNNING': running,
                    'PAUSE': pause_value}
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\nDone")
gs = dict(Pain=False, GO=False, old_GO=False, PAUSE=False, STOP=False, old_STOP=False, ABORT=True, old_ABORT=True,
          schedule_started=False, schedule_index=1, Global_cnt=0)
main_args = dict(old_GO=0, PAUSE=False, old_STOP=0, old_ABORT=1, schedule_started=False, old_schedule_started=False,
                 schedule_index=0, Global_cnt=0, P=Patm, elapsed_time=0, old_elapsed_time=0, pain_time=start_time,
                 old_pain=0)
...
for i in range(0, Max_num_schedules):
    ...
    print
    "Setting counter ", i, " to a value of ", Current_counter[i],
